...
#        self.threadlock = tl
        ...
        if platform.system() == 'Windows':
            temp = self.klc2layout.split('\\')
            self.klc2layout = '\\'.join(temp[:3]) + '\\klc2layout'
...
#        self.qr.put(('PRINT', 'backend {}'.format(SCRIPT_DIR)))
        while not self.exitFlag:
            ...
            if 'EXIT' in acommand:
                ...
                self.script_dir = json.loads(acommand[1])
#                self.qr.put(('PRINT',self.script_dir))
                self.qc.task_done()
            elif 'PROJECT' in acommand:
                self.project = json.loads(acommand[1])
                self.qc.task_done()
            elif 'LOAD_KLCFILES' in acommand:
#                self.template = acommand[1]
                self._load_klcfiles(acomand[1])
                self.qc.task_done()
...
#                break
    ...
    def _load_klcfiles(self, _project):
        self.project = json.loads(_project)
        the_path = self.project['currentSrcDir']
        klctree = self.project['klctree']
        self.qr.put(('PRINT', "I'm in _load_klcfiles"))
        for row in klctree:
            self.qr.put(('STATUS', 'Loading {}'.format(self.row[0])))
            self.klcfiles.append(Klc(the_path, row, self.qr))
        self.qr.put(('STATUS', 'Loading completed.'.format(self.row[0])))
...
#                            self.qr.put(('PRINT', '{} atag=>{}<'.format(k,atag)))
    ...
    def _on_copy_playlists(self, target):
        ...
        imglists = [i for i in os.listdir(images) \
                     if i.endswith('.png') or h.endswith('jpg')]
...
#            webpage = os.path.normpath('../{}/{}.html'.format(self.project, pid_item.tag))
            webpage = os.path.normpath('../{}/{}.html'.format(self.project, pid_item.text))
...
#                self.qr.put(('PRINT',child.text))
        ...
        for alink in page_links:
            linesout.append(ANAVLINK.format(os.path.normpath(alink[0]),alink[1]))
        linesout.extend(['    </div>',\
                         '  </nav>',\
                         '</body>',\
                         '</html>',\
...
'<h1 align="center"><b>{}</b></h1>'.format(os.path.basename(fileout)[:-5]), \
'', \
'<ul">', \
 ''])
        #open playlist
...
#        self.qr.put(("PRINT", tt))
        ...
        self.list_images = [self.hashed_graphics[m] for m in list(self.hashed_graphics.keys())]
        m = 0
...
#        fout.close()
        zipdir = os.path.normpath(self.script_dir + "/cssjs.zip")
        with zipfile.ZipFile(zipdir,"r") as zip_ref:
            zip_ref.extractall('{}/Temp/{}'.format(self.Pub2SD, self.project))        
