...
From desktop (Windows 64bit binary), double-click on the klc2layout icon.
...
Next klc2layout starts by asking three questions:
...
pre-loaded into klc2layout for any tags which are blank. No existing
...
klc2layout will generate unique names for every collection and MP3
...
be checked by klc2layout so ensure that it's characters are
...
telling klc2layout to retain them since both members of each pair are
...
Based on the order files are displayed klc2layout will create and
maintain unique
...
characters, when klc2layout creates the filename which will be
...
below it having their tags set to the unique name klc2layout
generated for them.
...
The 'playlists' generated by klc2layout can be copied either to
...
When you restart klc2layout and select an existing project all your
